...
l3-firewall's OPA Rego policies cover **17 attack categories** with **326 Go tests** and **76 Rego tests** plus **28 demo tests** across 15 internal packages and 12 standalone demos. All packages are **terminally hardened** — red-team rounds 17–37 confirmed zero new vulnerabilities across all 5 target packages (syncer, l2filter, persist, capture, threatintel) and their integration points; round 38 hardened the remaining engine integration points (audit, alert, geoip); round 39 hardened the engine's conntrack integration point (unbounded port-scan map); round 40 hardened the last zero-coverage hot-path packages (ratelimit, metrics) and the engine's parse-error fail-open path; round 41 closed the R40.1 key-space propagation gap in l2filter's ARP table (IPv4-mapped key split + ARP-spoof alias bypass) and made threatintel's `Remove()` version-independent.
...
### Verified Test Coverage (402 Go+Rego tests)
