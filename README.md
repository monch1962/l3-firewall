...
l3-firewall's OPA Rego policies cover **17 attack categories** with **323 Go tests** and **76 Rego tests** plus **28 demo tests** across 15 internal packages and 12 standalone demos. All packages are **terminally hardened** — red-team rounds 17–37 confirmed zero new vulnerabilities across all 5 target packages (syncer, l2filter, persist, capture, threatintel) and their integration points; round 38 hardened the remaining engine integration points (audit, alert, geoip); round 39 hardened the engine's conntrack integration point (unbounded port-scan map); round 40 hardened the last zero-coverage hot-path packages (ratelimit, metrics) and the engine's parse-error fail-open path.
...
### Verified Test Coverage (399 Go+Rego tests)
